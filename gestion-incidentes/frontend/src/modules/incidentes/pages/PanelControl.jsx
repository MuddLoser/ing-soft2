...
import {
  formalizarIncidente,
  editarIncidenteCompleto,
  buscarIncidentesEnBackend,
...
function PanelControl() {
  ...
  const [reincidenciaDetalle, setReincidenciaDetalle] = useState(null);
  ...
  const formatearFecha = (fechaOriginal) => {
    ...
    const partes = String(fechaOriginal).split("T");
  ...
  const obtenerPersonasFoco = (reincidencia) => {
    return (
      reincidencia.personas_foco ||
      (reincidencia.persona_foco ? [reincidencia.persona_foco] : [])
    );
  };
  ...
  const cargarDatos = () => {
    ...
    Promise.allSettled([
      buscarIncidentesEnBackend(terminoBusqueda, filtroFecha, filtroGravedad),
      obtenerReincidencias(),
    ])
      .then(([resultadoIncidentes, resultadoReincidencias]) => {
        if (resultadoIncidentes.status === "fulfilled") {
          setIncidentes(
            Array.isArray(resultadoIncidentes.value)
              ? resultadoIncidentes.value
              : []
          );
        } else {
          console.error(resultadoIncidentes.reason);
          setIncidentes([]);
          mostrarNotificacion(
            "No se pudieron cargar los incidentes registrados.",
            "error"
          );
        }

        if (resultadoReincidencias.status === "fulfilled") {
          setReincidencias(
            Array.isArray(resultadoReincidencias.value)
              ? resultadoReincidencias.value
              : []
          );
        } else {
          console.error(resultadoReincidencias.reason);
          setReincidencias([]);
          mostrarNotificacion(
            "No se pudieron cargar las reincidencias registradas.",
            "error"
          );
        }
      })
  ...
  useEffect(() => {
    cargarDatos();
  }, [terminoBusqueda, filtroFecha, filtroGravedad]);
  ...
  const abrirDetalleReincidencia = (reincidencia) => {
    setIncidenteDetalle(null);
    setReincidenciaDetalle(reincidencia);
    setModoEdicion(false);
    setToast({ mostrar: false, mensaje: "", tipo: "error" });
  };
  ...
  const handleGuardarCambios = async () => {
    ...
    if (!editPlan.trim() || !editSolucion.trim()) {
      mostrarNotificacion(
        "El plan de acción y la solución son obligatorios.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      const respuestaPython = await editarIncidenteCompleto(
        incidenteDetalle.id_i,
        datosAEnviar
      );
    ...
    <div className="page">
          ...
          >
            ...
            >
              <div>
                ...
                {formatearFecha(incidenteDetalle.fecha_i)}
              </div>
              <div>
                <strong>Lugar:</strong>{" "}
              ...
              <div style={{ textTransform: "capitalize" }}>
                ...
                {incidenteDetalle.gravedad || "No especificada"}
              </div>
              <div>
                <strong>Reportado por:</strong>{" "}
                {incidenteDetalle.reportado_por || "No registrado"}
              </div>
              <div>
            ...
            <div style={{ marginBottom: "24px" }}>
              ...
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                {incidenteDetalle.categorias &&
                incidenteDetalle.categorias.length > 0 ? (
            ...
            {incidenteDetalle.estado_i !== "Formalizado" ? (
              ...
              >
                El caso debe ser <strong>Formalizado</strong> antes de poder
                diseñar el plan de acción y las soluciones definitivas.
              </div>
            ) : (
              <div
                style={{
                ...
                {!modoEdicion ? (
                  <div>
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      <p
                        style={{
                          margin: 0,
                          fontSize: "14px",
                          color: incidenteDetalle.plan_accion_i
                            ? "var(--ink-800)"
                            : "var(--ink-400)",
                          fontStyle: incidenteDetalle.plan_accion_i
                            ? "normal"
                            : "italic",
                        }}
                      >
                        {incidenteDetalle.plan_accion_i ||
                          "No se ha registrado una estrategia o plan de acción de seguimiento aún."}
                      </p>
                    </div>
                    ...
                    <div style={{ marginBottom: "20px" }}>
                      ...
                      >
                        Solución y Medida Definitiva
                      </h4>
                      <p
                        style={{
                          margin: 0,
                          fontSize: "14px",
                          color: incidenteDetalle.solucion_i
                            ? "var(--ink-800)"
                            : "var(--ink-400)",
                          fontStyle: incidenteDetalle.solucion_i
                            ? "normal"
                            : "italic",
                        }}
                      >
                        {incidenteDetalle.solucion_i ||
                          "No se ha registrado la resolución definitiva adoptada."}
                      </p>
                    </div>
                  ...
                  >
                    ...
                    >
                      Actualizar Seguimiento y Datos Base
                    </h4>
                    ...
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      >
                        Solución Asignada (Resolución final)
                      </label>
                      <textarea
                        style={{
                          width: "100%",
      ...
      {reincidenciaDetalle && (
        ...
        >
          ...
          >
            {(() => {
              const personasFoco = obtenerPersonasFoco(reincidenciaDetalle);
              const personasInvolucradas =
                reincidenciaDetalle.personas_involucradas || [];
              const incidentesAsociados =
                reincidenciaDetalle.incidentes_asociados || [];
              const objetivos = reincidenciaDetalle.objetivos || [];
                ...
                <>
                  ...
                  >
                    <div>
                      ...
                      >
                        REINCIDENCIA {reincidenciaDetalle.id_r}
                      </span>
                      ...
                      >
                        Seguimiento de patrón de convivencia
                      </h3>
                    </div>

                    <button
                      type="button"
                      onClick={() => setReincidenciaDetalle(null)}
                      style={{
                        background: "transparent",
                        border: "none",
                        fontSize: "22px",
                  ...
                  >
                    <div>
                      ...
                      {formatearFecha(reincidenciaDetalle.fecha_revision)}
                    </div>
                    <div>
                      <strong>Responsable:</strong>{" "}
                      {reincidenciaDetalle.encargado_seguimiento ||
                        "No registrado"}
                    </div>
                    <div>
                      <strong>Incidentes asociados:</strong>{" "}
                      {incidentesAsociados.length}
                    </div>
                  </div>
                  ...
                  {personasInvolucradas.length > 0 && (
                    <div style={{ marginBottom: "20px" }}>
                      ...
                      >
                        Personas involucradas secundarias
                      </h4>
                      <div
                        style={{
                          display: "flex",
                      ...
                      >
                        {personasInvolucradas.map((persona, idx) => (
                          <span
                            key={idx}
                            style={{
                              background: "var(--bg-2)",
      ...
      {toast.mostrar && (
        <div
          style={{
            ...
            border: `1px solid ${
              toast.tipo === "error" ? "#f87171" : "#4ade80"
            }`,
            padding: "16px 24px",
            borderRadius: "8px",
