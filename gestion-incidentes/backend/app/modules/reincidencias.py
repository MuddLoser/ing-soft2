...
class Reincidencia:
    def __init__(self, id_r, persona_foco, personas_involucradas, incidentes_asociados, encargado_seguimiento, fecha_revision, objetivos, analisis):
        self.id_r = id_r
...
class ReincidenciaRepository:
    def cargar_todas(self):
        if not os.path.exists(ARCHIVO_REINCIDENCIAS):
            return []
        try:
            with open(ARCHIVO_REINCIDENCIAS, 'r', encoding='utf-8') as archivo:
                datos = json.load(archivo)
                return [Reincidencia.from_dict(d) for d in datos]
        except json.JSONDecodeError:
            return []

    def guardar_todas(self, reincidencias):
        with open(ARCHIVO_REINCIDENCIAS, 'w', encoding='utf-8') as archivo:
            json.dump([r.to_dict() for r in reincidencias], archivo, indent=4, ensure_ascii=False)
...
class GestorReincidencias:
    ...
    def crear_reincidencia(self, persona_foco, personas_involucradas, incidentes, encargado, fecha_revision, objetivos, analisis):
        nuevo_id = max([r.id_r for r in self.reincidencias], default=0) + 1
        nueva = Reincidencia(nuevo_id, persona_foco, personas_involucradas, incidentes, encargado, fecha_revision, objetivos, analisis)
        self.reincidencias.append(nueva)
        self.repo.guardar_todas(self.reincidencias)
        return nueva
